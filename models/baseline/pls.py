...
MODEL_NAME = 'pls_baseline'


class _PLSWrapper(BaseEstimator, RegressorMixin):
    """Adapter: caps n_components to feasible size and returns 1-D predictions."""
...
def run_pls_baseline(file_path=None):
    setup_plot_style()
    ...
    metrics = evaluate_and_plot(y_true, y_pred, 'PLS (Baseline)',
                                os.path.join(BASELINE_OUTPUT_DIR, '5_pls.png'))
